/**
    ...
    packet 1 -> id for 2 byte | size of size for 1 bytes | size of msg for ?? bytes | totals at max 1024 bytes
    packet 2 to n -> msg with a number of bytes equal to size
        ...
        0 is get changablility buffer
        1 is get sim width, heigh, depth
        ...
        254 is reset
        255 is quit
...
template<typename T>
class CommunicatorConnection: public Poco::Net::TCPServerConnection 
{
    private:
        Simulation * sim // a pointer to the simulation;
        std::atomic<bool> quit; // a boolean to quit the program
    ...
    public:
    CommunicatorConnection(const Poco::Net::StreamSocket& s, Simulation * pointer_to_simulation): TCPServerConnection(s) 
    {
        this->sim = pointer_to_simulation;
        
        sycl::range<3> tempDims = this->sim->get_dimensions();

        this->width = tempDims.get(0)
        this->height = tempDims.get(1);
        this->depth = tempDims.get(2);
    }
    ...
    void run() 
    {
        ...
        while(!exit)
        {
            try {
                ...
                char send_buffer[send_buffer_length]; // size is arbitrary rn as idk what it should be 
